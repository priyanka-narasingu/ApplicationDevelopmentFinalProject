...
{
 public   class DeptHeadViewRequisitionHistoryController
    {
        ...
        public List<Request> getRequestByUser(String username,string status)
        {
            ...
            var z = from y in ent.Requests
                    ...
                    select y;
            requestrownumber = z.Count();
            return req = z.ToList();
    
        
        }
        public List<Request> getRequestByDate(string username,string status, DateTime date)
        {
            Employee emp = new Employee();
            emp = ent.Employees.First(x => x.UserName == username);
            List<Employee> emplist = new List<Employee>();
            ...
            var z = from y in ent.Requests
                    join j in ent.Employees
                    on y.EmployeeID equals j.EmployeeID
                    where j.DeptCode == st && y.RequestStatus.Trim() == status && y.DateCreated>=date && y.DateCreated<nextdate
                    select y;
        ...
        {
            DateTime date;
            Request req = ent.Requests.First(x=>x.RequestStatus=="Pending Approval");
            return date = Convert.ToDateTime(req.DateCreated);
        }
    }
    }
